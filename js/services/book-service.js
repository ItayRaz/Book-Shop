...
let books = [
  ...
  function getBooks() {
    let newBooks = utilService.load(STORAGE_KEY);
    if(!newBooks || books.length === 0){
      newBooks = books;
      for(var i = 0; i < books.length; i++){
        books[i]['review'] = [];
      }
      utilService.store(STORAGE_KEY, newBooks)
    }
    books = newBooks;

      return Promise.resolve(books);
  }
  findBook('OXeMG8wNskc')
  ...
  function searchBook(str) {
    let apiUrl = `https://www.googleapis.com/books/v1/volumes?q=${str}&key=AIzaSyCbhn96qgXx4hd50UPQZexxY5UqPA8jBuQ`
  ...
  function addBook(bookUrl) {
    return axios.get(bookUrl)
        ...
        .then(book => {
          ...
          if(!book.saleInfo.listPrice)book.saleInfo.listPrice = {
            "amount": 186,
            "currencyCode": "ILS",
            "isOnSale": true
          }
          return {
            ...
            "thumbnail": book.volumeInfo.imageLinks.medium,
